...
void fillin_wagner(node2 *p,long fullset,boolean full,bitptr wagner,bitptr zeroanc)
{
...
void postorder_wagner(node2 *p, long fullset, boolean full, bitptr wagner,
                        bitptr zeroanc)
{
  /* traverses a binary tree, calling PROCEDURE fillin_wagner at a
     node's descendants before calling fillin_wagner at the node2 */
  /* used in mix & penny */
  if (p->tip)
    return;
  postorder_wagner(p->next->back, fullset, full, wagner, zeroanc);
  postorder_wagner(p->next->next->back, fullset, full, wagner, zeroanc);
  if (!p->visited) {
    fillin_wagner(p, fullset, full, wagner, zeroanc);
                        ...
                        bitptr zeroanc)
{
  ...
  fillin_wagner(r, fullset, full, wagner, zeroanc);
}  /* filltrav */


void hyprint_wagner(struct htrav_vars2 *htrav, boolean unknown, boolean noroot,
...
void hyptrav_wagner(node2 *r_, boolean unknown, bitptr dohyp, long fullset,
        ...
        gbit *garbage)
{
  ...
  hyprint_wagner(&vars,unknown, noroot, didreroot, wagner, guess);
  if (!vars.r->tip) {                
    hyptrav_wagner(vars.r->next->back,unknown,dohyp, fullset, noroot,didreroot,
               wagner, zeroanc, oneanc, treenode, guess, garbage);
    hyptrav_wagner(vars.r->next->next->back, unknown,dohyp, fullset, noroot,
...
void hypstates_wagner(long fullset, boolean full, boolean noroot, boolean didreroot,
                        ...
                        pointptr2 treenode, Char *guess, gbit *garbage)
{
  ...
  hyptrav_wagner(root,unknown,dohyp, fullset, noroot, didreroot, wagner,
                zeroanc, oneanc, treenode, guess, garbage);
  free(dohyp);
}  /* hypstates_wagner */


void drawline_wagner(long i, double scale, node2 *root)
{
