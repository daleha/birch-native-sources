...
/*  $Id: initfa.c 877 2011-11-17 16:25:16Z wrp $ */
/*  $Rev: 877 $  */
...
#ifdef PCOMPLIB
char *verstr="36.3.5b Nov, 2011 MPI";
#else
char *verstr="36.3.5b Nov, 2011";
#endif
	  ...
	  struct mngmsg *m_msg, struct pstruct *ppst)
{
  ...
  case 'c':
    ...
    if (*optarg != '\0' && pgm_def_arr[pgm_id].ktup > 0) {
      sscanf (optarg, "%f %f", &tmp_f, &tmp_f1);
      if (tmp_f > 1.0) {
  ...
  case 'E':
    if (strchr(optarg,' ')) {	/* check for 1 or 2 values */
      sscanf(optarg,"%f %f",&tmp_e_cut, &tmp_e_rep);
