...
/*  $Id: cal_cons.c 739 2011-05-17 19:26:35Z wrp $ */
/* $Revision: 739 $  */
...
static void update_code(char *al_str, int al_str_max, int op, int op_cnt, int show_code);
  ...
  while (i0 < a_res->max0 || i1 < a_res->max1) {
	...
	if (aa1a) *sp1a++ = ann_arr[aa1a[i1]];
      ...
      *sp1 = sq[aa1p[i1++]];
      ...
      aln->nmismatch++; /* always increment mismatch, decrement for identities */
...
static void
update_code(char *al_str, int al_str_max, int op, int op_cnt, int show_code) {
  char *ori_code = "=-+*";
  char *cigar_code = "MIDM";
	      ...
	      int show_code)
{
  ...
  */
  ...
  rp = a_res->res;
  lenc = aln->nident = aln->nmismatch = aln->nsim = aln->npos = aln->ngap_q = aln->ngap_l = aln->nfs = op = p_op = 0;
  op_cnt = 0;
  ...
  while (i0 < a_res->max0 || i1 < a_res->max1) {
    if (op == 0 && *rp == 0) {	/* in a match run (op==0) and current code is a match */
	...
	sim_code = M_POS;
      ...
      else if (ppst->pam2[0][aa0[i0]][aa1p[i1]]==0) { 
	aln->nsim++;
	sim_code = M_ZERO;
      }
	...
	if (sp0 != '*' && sp1 != '*') {
	  if (p_op == 3) {
	    update_code(al_str,al_str_n-strlen(al_str),p_op,op_cnt, show_code);
	    op_cnt = 1; p_op = 0;
	  }
	  else {op_cnt++;}
	}
	...
	aln->nident++;
	sim_code = M_IDENT;
	aln->nmismatch--;	/* decrement, was pre-incremented */
      }
	    ...
	    struct f_struct *f_str)
{
  ...
  while (i0 < a_res->max0 || i1 < a_res->max1) {
    if (op == 0 && *rp == 0) {
      ...
      aln->nmismatch++;
      if (toupper(sp0) == toupper(sp1)) {aln->nident++; aln->nmismatch--;}
      else if (ppst->nt_align) {
	if ((toupper(sp0)=='T' && toupper(sp1)== 'U')||
	  (toupper(sp0)=='U' && toupper(sp1)=='T')) {aln->nident++; aln->nmismatch--;}
	else if (toupper(sp0) == 'N') aln->ngap_q++;
	else if (toupper(sp1) == 'N') aln->ngap_l++;
      }
    }
