...
/* From input file "d4hom.p" */


/* ********************************************************  */
/*                                                           */
/*   D4HOM     VERSION  8/13/2001, Standard Pascal           */
/*             Brian Fristensky                              */
/*             Dept. of Plant Science                        */
/*             University of Manitoba                        */
/*             Winnipeg, MB R3T 2N2  CANADA                  */
/*                                                           */
/* Copyright (c) 1984,1986,1987, 1994  by Brian Fristensky.  */
...
#define VERSION         "D4HOM         Version  8/13/2001"
...
Static NODE *TETRANUC[(long)G - (long)T + 1][(long)G - (long)T + 1]
	    [(long)G - (long)T + 1][(long)G - (long)T + 1];
Static NODE *FREELIST;
Static Char NUCHAR[16];
Static long STARTX, FINISHX, STARTY, FINISHY, HOMRANGE, MINPER, COMPFACT,
	    LINESIZE, MAXSCORE, FOURMATCH, MINSCORE;
Static double SCALEFAC, MAXFACTOR, TEMP;
Static long SUBSCORE[MAXRANGE];
Static Char KIND;
  ...
  *SEQLEN = 0;
  ...
  while (!BUFEOF(SFILE->f)) {
	...
	CH = ' ';
	  ...
	  CH == 'a' || CH == 'A') {
/* p2c: d4hom.p, line 544: 
 * Note: Line breaker spent 1.0 seconds, 5000 tries on line 842 [251] */
	if (*SEQLEN < MAXSEQ - 2) {
	  /*write(CH);*/
	  (*SEQLEN)++;
...
Static Void INITIALIZE()
{
  ...
  /* Initialize TETRANUC table to nil */
  for (N1 = T; (long)N1 <= (long)G; N1 = (NUCLEOTIDE)((long)N1 + 1)) {
	...
	for (N4 = T; (long)N4 <= (long)G; N4 = (NUCLEOTIDE)((long)N4 + 1))
	  TETRANUC[(long)N1 - (long)T][(long)N2 - (long)T][(long)N3 - (long)T]
	    [(long)N4 - (long)T] = NULL;
      }
    }
  }
...
/*******************************************************************/
/* Calculate a table of scores for identities at each distance from*/
/* the central triplet in a local similarity.                      */
/*******************************************************************/
Static Void CALCSCORES()
{
  ...
  FOURMATCH = SUBSCORE[0] * 2 + SUBSCORE[1] * 2;
...
/*  Make a table of locations of tetranucleotides in SEQX.  Each      */
/*  tetrad has a stack of nodes, each of which holds a location in    */
/*  SEQX at which the tetranucleotide occurs.                         */
...
struct LOC_QUICKSEARCH *LINK;
{
  ...
  for (V.I = RIGHT; V.I >= LEFT; V.I--) {
    N1 = SEQX[V.I + MAXRANGE];
    N2 = SEQX[V.I + MAXRANGE + 1];
    N3 = SEQX[V.I + MAXRANGE + 2];
    N4 = SEQX[V.I + MAXRANGE + 3];
    if ((long)N1 < (long)N && (long)N2 < (long)N && (long)N3 < (long)N &&
	(long)N4 < (long)N)
      ADDNODE(&TETRANUC[(long)N1 - (long)T][(long)N2 - (long)T]
	      [(long)N3 - (long)T][(long)N4 - (long)T], &V);
  }
}  /*MAKETABLE*/

/* At each occurrence of the tetrad  in SEQX, compare a region  */
...
/* Push the linked-list (if there is one) for each TETRANUC to */
/* the top of the FREELIST and reset TETRANUC to nil.          */
...
struct LOC_QUICKSEARCH *LINK;
{
  ...
  for (N1 = T; (long)N1 <= (long)G; N1 = (NUCLEOTIDE)((long)N1 + 1)) {
    for (N2 = T; (long)N2 <= (long)G; N2 = (NUCLEOTIDE)((long)N2 + 1)) {
      for (N3 = T; (long)N3 <= (long)G; N3 = (NUCLEOTIDE)((long)N3 + 1)) {
	for (N4 = T; (long)N4 <= (long)G; N4 = (NUCLEOTIDE)((long)N4 + 1)) {
	  if (TETRANUC[(long)N1 - (long)T][(long)N2 - (long)T]
	      [(long)N3 - (long)T][(long)N4 - (long)T] != NULL) {
	    HEAD = TETRANUC[(long)N1 - (long)T][(long)N2 - (long)T]
	      [(long)N3 - (long)T][(long)N4 - (long)T];
	    TAIL = HEAD;
	    while (TAIL->NEXT != NULL)
	      TAIL = TAIL->NEXT;
	    TAIL->NEXT = FREELIST;
	    FREELIST = HEAD;
	    TETRANUC[(long)N1 - (long)T][(long)N2 - (long)T]
	      [(long)N3 - (long)T][(long)N4 - (long)T] = NULL;
	  }
	}
      }
 ...
 * Note: Using % for possibly-negative arguments [317] */
    ...
    for (V.POSY = STARTY; V.POSY <= FORLIM; V.POSY++) {
      N1 = SEQY[V.POSY + MAXRANGE];
      N2 = SEQY[V.POSY + MAXRANGE + 1];
      N3 = SEQY[V.POSY + MAXRANGE + 2];
      N4 = SEQY[V.POSY + MAXRANGE + 3];
      if ((long)N1 < (long)N && (long)N2 < (long)N && (long)N3 < (long)N &&
	  (long)N4 < (long)N)
	SEARCHFOR(TETRANUC[(long)N1 - (long)T][(long)N2 - (long)T]
		  [(long)N3 - (long)T][(long)N4 - (long)T], &V);
      INDEX++;
      if (INDEX == COMPFACT) {
	if (K_ < 9) {
    ...
    if (RIGHT > RIGHTLIM)
      RIGHT = RIGHTLIM;
    if (CHOICE == 6)   /*page break for file output only*/
      fprintf(V.OUTFILE->f, "\f");
  }  /* while LEFT <= RIGHTLIM */
