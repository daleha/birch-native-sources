...
(* BEGIN MODULE VERSION *)
        VERSION= 'SYSVMODS     Version  9/12/91';
(* END MODULE VERSION *)
...
procedure GETFILE(var F:text; FTYPE:char; var FILENAME:LINE);
  ...
  begin
    repeat
      ...
      case FTYPE of
        'I':begin (* input file *)
(*!!!*)       while not exists(FILENAME.STR) do begin
...
(*!!!*) 'O':begin
            repeat
              OKAY:=true;
              if exists(FILENAME.STR) then begin
                repeat  
...
(*!!!*) if not openok(F) then begin
               writeln('***FILE CAN NOT BE OPENED.');
               writeln('Enter another filename:')
               end        
     until openok(F) (*!!!*)
  end;  (* GETFILE *)
          ...
          with ORI do begin
           ...
           if not eof(SFILE) then begin
             ...
             if SAMEWORD(ID,LOCUS) then begin
                ...
                READWORD(SFILE,ID);
                CH:=' ';I:=0;
                while (I < 14) and (not eoln(SFILE)) do begin
                  read(SFILE,CH);
                  I:=I+1
                  end;
                if (CH='C') or (CH='c') then CIRCULAR:=true
                else CIRCULAR:=false
