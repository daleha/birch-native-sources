...
*)
  ...
  program MULTIDIGEST(input, output (*,INFILE,OUTFILE*));
(*!!! Some Pascals may require file parameters in program heading *)
  ...
  const MAXSITES=  100000;  (*Max. number of sites in all digests *)
        ...
        MAXLINE = 150;
        VERSION = 'MULTIDIGEST       Version 29 Mar 2006';
...
(* END MODULE GETINTEGER         VERSION= 'SUNMODS     Version  6/26/01'; *)
  ...
  procedure READSITES;
    ...
    begin
      ...
      (* Read title box *)
      readln(INFILE);readln(INFILE);readln(INFILE);
...
WRITEWORD(output,DUMMY, DUMMY.LEN); write(output,' ');
      ...
      READWORD(INFILE,TOPOLOGY);READWORD(INFILE,DUMMY);
(*!!!*) WRITEWORD(output, TOPOLOGY, TOPOLOGY.LEN); write(output,' ');
WRITEWORD(output,DUMMY, DUMMY.LEN); write(output,' ');
      if TOPOLOGY.STR[1]='C' then CIRCULAR:= true
      else CIRCULAR:= false;
      readln(INFILE,SEQLEN);
(*!!!*)writeln(output,SEQLEN);
      
      (* Read parameter box *)
      for I:= 1 to 13 do readln(INFILE);
...
(*        read(INFILE,CUT); read(INFILE,CH);
	  ...
	  (* Check to see if fragments are shown. BACHREST will
	     ...
	     this enzyme. *)
	  while INFILE^=' ' do read(INFILE,CH);
	  if INFILE^='(' then begin FRAGSFOUND:= false; readln(INFILE) end
	  else FRAGSFOUND:= true;   
	     
          if ((NUMSITES > 0) and FRAGSFOUND) then begin
...
(* END MODULE STARTUP         VERSION= 'SUNMODS     Version  6/26/01'; *)
      ...
      repeat
        ...
        writeln('MULTIDIGEST','MAIN MENU':25);
        WRITELINE(output,HLINE,80);writeln;
