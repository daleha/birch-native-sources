...
(*!!!*)  program D4HOM(input, output (*, SFILEX,SFILEY,OUTFILE*));
...
(* END MODULE REALLIMITS         VERSION= 'SUNMODS     Version  6/26/01'; *)
        ...
        MAXRANGE= 30;
        MAXLINE =  150;
        VERSION = 'D4HOM         Version  8/13/2001';
...
(* END MODULE TYPE.LINE         VERSION= 'SUNMODS     Version  6/26/01'; *)
  ...
  var SFILEX,SFILEY,OUTFILE: text;
      ...
      LENX,LENY    : integer;
      NAMEX,NAMEY  : WORD;
      TETRANUC     : array[T..G,T..G,T..G,T..G] of NP;
      FREELIST     : NP;
      NUCHAR       : array[NUCLEOTIDE] of char;
      STARTX,FINISHX,
      STARTY,FINISHY,
      HOMRANGE,MINPER,COMPFACT,LINESIZE,
      MAXSCORE,FOURMATCH,MINSCORE: integer;
  ...
  procedure INITIALIZE;
    ...
    begin
      ...
      for N1:= T to G do
        for N2:= T to G do
          for N3:= T to G do
            for N4:= T to G do TETRANUC[N1,N2,N3,N4]:= nil
    end; (* INITIALIZE *)
...
(* END MODULE GETCHAR         VERSION= 'SUNMODS     Version  6/26/01'; *)
  ...
  procedure CALCSCORES;
    ...
    begin
      ...
      FOURMATCH:= (2*SUBSCORE[1]) + (2*SUBSCORE[2]);
   ...
   procedure MAKETABLE(LEFT,RIGHT:integer);
     ...
     begin
       for I:= RIGHT downto LEFT do begin
         N1:= SEQX[I]; N2:= SEQX[I+1]; N3:= SEQX[I+2]; N4:= SEQX[I+3];
         if ((N1<N) and (N2<N) and (N3<N) and (N4<N)) then
            ADDNODE(TETRANUC[N1,N2,N3,N4])
         end
    end; (*MAKETABLE*)
   ...
   procedure SEARCHFOR(N:NP);
     ...
     begin
       while N <> nil do begin
         POSX:= N^.POS;
         SCORE:= FOURMATCH;
         LX:= POSX-1; RX:= POSX+4; LY:=POSY-1; RY:=POSY+4;
         DISTANCE:= 3;
    ...
    begin
      for N1:= T to G do
        for N2:= T to G do
          for N3:= T to G do
            for N4:= T to G do
              if TETRANUC[N1,N2,N3,N4] <> nil then begin
                HEAD:= TETRANUC[N1,N2,N3,N4];
                TAIL:= HEAD;
                while TAIL^.NEXT <> nil do TAIL:= TAIL^.NEXT;
                TAIL^.NEXT:= FREELIST;
                FREELIST:= HEAD;
                TETRANUC[N1,N2,N3,N4]:= nil
                end
    end; (*RIDOF*)
    ...
    begin (* QUICKSEARCH *)
      ...
      while LEFT <= RIGHTLIM do begin
        ...
        for POSY:= STARTY to FINISHY do begin
          N1:=SEQY[POSY]; N2:=SEQY[POSY+1]; N3:=SEQY[POSY+2]; N4:=SEQY[POSY+3];
          if ((N1<N) and (N2<N) and (N3<N) and (N4<N)) then
            SEARCHFOR(TETRANUC[N1,N2,N3,N4]);
          INDEX:= INDEX+1;
          ...
          if RIGHT > RIGHTLIM then RIGHT:= RIGHTLIM;
          if CHOICE=6 then page(OUTFILE) (*page break for file output only*)
          end; (* while LEFT <= RIGHTLIM *)
