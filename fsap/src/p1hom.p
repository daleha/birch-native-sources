(*$DEBUG- *)
  ...
  program P1HOM(input, output (*,SFILEX,SFILEY,OUTFILE*));
...
(* END MODULE READPRO         VERSION= 'SUNMODS     Version  9/12/91'; *)
    ...
    procedure INITIALIZE;
      ...
      AACHAR[UNKX]:='X';AACHAR[UNKY]:='X';
        ...
        for A1:= GLY to ARG do PEPTIDE[A1]:= nil
   ...
   procedure MAKETABLE(LEFT,RIGHT:integer);
     ...
     begin
       for I:= RIGHT downto LEFT do begin
         A1:= SEQX[I];
         if A1<=ARG then ADDNODE(PEPTIDE[A1])
         end
    end; (*MAKETABLE*)
   ...
   procedure SEARCHFOR(N:NP);
     ...
     begin
       while N <> nil do begin
         POSX:= N^.POS;
         SCORE:= ONEMATCH;
         LX:= POSX-1; RX:= POSX+1; LY:=POSY-1; RY:=POSY+1;
         DISTANCE:= 1;
    ...
    begin
      ...
      while LEFT <= RIGHTLIM do begin
        ...
        for POSY:= STARTY to FINISHY do begin
          A1:=SEQY[POSY];
          if A1<=ARG then SEARCHFOR(PEPTIDE[A1]);
          INDEX:= INDEX+1;
